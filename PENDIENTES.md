...
- ✅ El repo hermano (`iita-gsc-mcp`, mismo Dockerfile/patrón/versión de Python) se reconstruyó
  en Docker: **con** `MCP_AUTH_TOKEN` arranca y `/mcp` sin credencial da 401; **sin** el token
  sale con `ConfigError` y no arranca. *(No se repitió el build end-to-end en este repo; el
  código de auth es el mismo patrón portado.)*
- ❌ Producción devuelve **502** en `/health` y `/mcp`, mismo síntoma que gsc: **el proceso no
  está viendo `MCP_AUTH_TOKEN` en runtime**.
...
2026-08-04 reproduciendo el bypass en el repo hermano: un cliente que solo conoce la URL
(nunca el `MCP_AUTH_TOKEN`) corre el flujo OAuth (`/oauth/register` → `/authorize` → `/token`) y
obtiene un token válido, porque `/authorize` auto-consiente sin autenticar al dueño. En modo
OAuth el `MCP_AUTH_TOKEN` solo firma los JWT internos, no controla acceso.
...
En `C:\Users\violl\IITA-MARKETING\.mcp.json`, entrada `iita-ga4`.
...
```bash
curl -s -o /dev/null -w '%{http_code}\n' -X POST https://web-production-bc44d.up.railway.app/mcp \
