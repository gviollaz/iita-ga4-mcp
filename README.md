...
MCP server providing GA4 reporting data for iita.com.ar.
...
conociera la URL de Railway podía hacer `tools/list` y leer los datos de GA4 de
iita.com.ar (sesiones, páginas, fuentes de tráfico, geografía, tiempo real).
Verificado el 2026-08-03 con un `tools/call` sin credenciales contra el deploy
real, que devolvió datos de tráfico verdaderos.
...
"iita-ga4": {
  "command": "npx",
  "args": ["-y", "mcp-remote", "https://web-production-bc44d.up.railway.app/mcp"]
}
```
...
"iita-ga4": {
  "command": "npx",
  "args": ["-y", "mcp-remote", "https://web-production-bc44d.up.railway.app/mcp/EL_TOKEN"]
}
```
...
```bash
curl -s -o /dev/null -w '%{http_code}\n' -X POST https://web-production-bc44d.up.railway.app/mcp -H 'Content-Type: application/json' -H 'Accept: application/json, text/event-stream' -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-03-26","capabilities":{},"clientInfo":{"name":"c","version":"1"}}}'
```
...
| Tool | Description |
|------|-------------|
| `ga4_run_report` | Reporte libre: dimensiones, métricas, rango y filtros |
| `ga4_realtime_report` | Usuarios activos ahora |
| `ga4_traffic_sources` | Canales, fuentes y medios |
| `ga4_top_pages` | Páginas más vistas con engagement |
| `ga4_daily_trend` | Tendencia diaria de sesiones/usuarios/vistas |
| `ga4_landing_pages` | Rendimiento de páginas de entrada |
| `ga4_device_geo` | Corte por dispositivo, país o ciudad |
