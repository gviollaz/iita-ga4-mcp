"""IITA GA4 MCP server entry point — v0.2.0.

Adds authentication to the Analytics 4 MCP server. Until v0.2.0 this
server answered every request unauthenticated: anyone who knew the Railway
URL could call `tools/list` and read the GA4 analytics data of iita.com.ar.
...
_VERSION = "0.2.0"
SERVICE_NAME = "iita_ga4_mcp"
DOCS_URL = "https://github.com/gviollaz/iita-ga4-mcp"
...
def _require_auth_token() -> str:
    ...
    if not token:
        raise ConfigError(
            ...
            "Refusing to start: without it the server would expose GA4 "
            "analytics data to anyone who knows the URL."
        )
    return token
